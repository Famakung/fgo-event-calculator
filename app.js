...
(function () {
  ...
  const TraitMatcher = {
    matches(servantTraits, ce) {
      ...
      if (ce.traitGroups.length > 0) {
        return ce.traitGroups.every((group) => group.some((t) => servantTraits.includes(t)));
      }
  ...
  const DOMFactory = {
    ...
    createIcon(tier, size = "normal") {
      const container = this.el("div", size === "normal" ? "material-icon" : "mini-icon");
    ...
    load() {
      ...
      this.servants = Object.entries(map)
        .map(([id, data]) => {
          ...
          const hasAscensions =
            !isArray && typeof rawTraits === "object" && Object.keys(rawTraits).some((k) => k !== "base");
  ...
  const ServantSelector = {
    ...
    _buildClassFilter(container) {
      ...
      const buildRow = (classes) => {
        ...
        classes.forEach((cls) => {
          const btn = DOMFactory.el("div", "servant-class-btn" + (selected.has(cls.id) ? " active" : ""));
    ...
    _buildRarityFilter(container) {
      ...
      rarities.forEach((rarity) => {
        const btn = DOMFactory.el("div", "servant-rarity-btn" + (selected.has(rarity.id) ? " active" : ""));
  ...
  const CEFilterPicker = {
    ...
    renderGrid(ces) {
      ...
      ces.forEach((ce) => {
        ...
        const item = DOMFactory.el(
          "div",
          "servant-picker-item ce-filter-picker-item" + (isSelected ? " selected" : ""),
        );
  ...
  const CEServantOverlap = {
    ...
    _buildCountFilter(container, preFiltered) {
      ...
      for (let n = 1; n <= Math.max(...availableCounts); n++) {
        ...
        const btn = DOMFactory.el("div", "cefilter-match-count-btn" + (selected.has(n) ? " active" : ""));
    ...
    _buildClassFilter(container, availableClassIds) {
      ...
      const buildRow = (classes) => {
        ...
        classes.forEach((cls) => {
          ...
          const btn = DOMFactory.el(
            "div",
            "ceoverlap-class-btn" + (this._classFilters.includes(cls.id) ? " active" : ""),
          );
    ...
    _buildRarityFilter(container, availableRarityIds) {
      ...
      rarities.forEach((rarity) => {
        ...
        const btn = DOMFactory.el(
          "div",
          "ceoverlap-rarity-btn" + (this._rarityFilters.includes(rarity.id) ? " active" : ""),
        );
  ...
  const CEFilterApp = {
    ...
    buildClassFilter() {
      ...
      const buildRow = (classes) => {
        ...
        classes.forEach((cls) => {
          const btn = DOMFactory.el("div", "cefilter-class-btn" + (selected.has(cls.id) ? " active" : ""));
          btn.dataset.traitId = cls.id;
          const img = DOMFactory.el("img", "", {
            src: `icons/classes/${cls.icon}.webp`,
            alt: cls.label,
    ...
    buildRarityFilter() {
      ...
      rarities.forEach((rarity) => {
        const btn = DOMFactory.el("div", "cefilter-rarity-btn" + (selected.has(rarity.id) ? " active" : ""));
        btn.dataset.traitId = rarity.id;
        btn.textContent = rarity.label;
        btn.addEventListener("click", () => {
          if (selected.has(rarity.id)) {
    ...
    filterByCEs(results) {
      ...
      if (this.state.mode === "all") {
        return results.filter((entry) => selectedCEObjs.every((sel) => entry.matchingCEs.some((m) => m.id === sel.id)));
      }
      ...
      return results.filter((entry) => {
        const matchedCount = selectedCEObjs.filter((sel) => entry.matchingCEs.some((m) => m.id === sel.id)).length;
    ...
    renderChips() {
      ...
      if (this.state.selectedCEs.length === 0) {
        ...
        placeholder.textContent =
          "No craft essences selected \u2014 showing all servants. Add Craft Essence to filter.";
    ...
    renderResults(ceFiltered) {
      ...
      pageSlice.forEach(
        ({ servant, matchingCEs, allTraitNames, matchedAscensions, baseMatchesAll, primaryAscension }) => {
          ...
          const imgAsc =
            !baseMatchesAll && matchedAscensions.length > 0 ? primaryAscension || matchedAscensions[0] : null;
  ...
  const BondApp = {
      ...
      for (let i = 0; i < count; i++) {
        ...
        if (ce) {
          ...
          if (ceBonusText !== null) {
            ...
            const groups = ce.traitGroups
              .map((group) => group.map((t) => TraitNames[t] || t).join(" or "))
              .join(" and ");
    ...
    buildServantSlots() {
      ...
      for (let i = 0; i < count; i++) {
        ...
        const slotClass =
          i < BOND_CONSTANTS.FRONTLINE_SIZE
